...
def split_pdf(pngscale):
    """
    take the combined PDF file and split it into two png file per page
    """
...
def create_mixup_dungeons(characters,dungeons,n_pages):
    """
    make all the pages
    """
    for ii in range(n_pages):
        for jj in range(n_pages):
            ...
            dungeonpage = merge(im1, im2,pngscale)
...
def main():
    ...
    # name all the characters and dungeons here for cutely named files.
    # This is the core set and the first nine weeks of the yearlong set.
    # You need to make sure the pages of the PDF are in the same order
    #  as this or your names will all come out wrong.
    # If you *only* have the year long set,
    # delete the first six names from each list; they're the core items.
    characters = [
    ...
    n_pages = split_pdf(pngscale)
    create_mixup_dungeons(characters,dungeons,n_pages)
